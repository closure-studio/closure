    ...
    <KeyboardAvoidingView behavior={process.env.EXPO_OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
      <YStack grow={1} minH={0}>
        <ScrollView grow={1} keyboardShouldPersistTaps="handled" contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} contentContainerStyle={{ grow: 1 }}>
          <YStack width="100%" maxW={1120} minH="100%" grow={1} self="center" px="$4" pt="$3.5" pb="$2.5" $large={{ px: '$6', pt: '$5', pb: '$4' }}>
            <XStack grow={1} items="center" justify="center" flexDirection="column" px="$2" py="$3" gap="$4" $large={{ flexDirection: 'row', gap: '$12', px: '$5', py: '$5' }}>
              <YStack position="relative" width="100%" maxW={420} transition={reducedMotion ? '0ms' : '500ms'} enterStyle={reducedMotion ? null : { opacity: 0, y: 18 }} opacity={1} y={0} gap="$3" $large={{ grow: 1 }}>
              ...
              <YStack width="100%" maxW={440} $large={{ grow: 1 }}>
                <LoginForm
            ...
            <XStack flexDirection="column" items="center" gap="$1" pt="$3" borderTopWidth={1} borderColor="$appBorder" $large={{ flexDirection: 'row', justify: 'space-between', items: 'center' }}>
