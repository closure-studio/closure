  ...
  return (
    <TerminalPanel transition="quickLessBouncy" enterStyle={{ opacity: 0, y: 12 }} exitStyle={{ opacity: 0, scale: 0.9 }} opacity={1} y={0} scale={1} width="48.7%" minW={140} grow={1} p={12} tone={material.tier >= 5 ? 'warning' : material.tier === 4 ? 'cyan' : 'default'} $md={{ width: '31.5%' }} $lg={{ width: '23.5%' }} $xl={{ width: '18.5%' }}>
      <XStack items="center" justify="space-between">
        <MonoText size="$1" color={material.tier >= 5 ? '$terminalWarning' : material.tier === 4 ? '$terminalCyan' : '$terminalMuted'}>{t('inventory.tier', { tier: material.tier })}</MonoText>
        {hasShortage ? <MonoText size="$1" color="$terminalWarning">{t('inventory.shortage', { count: material.needed - material.owned })}</MonoText> : null}
      </XStack>
      <TerminalText mt={4} size="$3" fontWeight="700">{material.name}</TerminalText>
      <XStack mt={8} items="baseline" gap="$1">
        <TerminalText size="$5.5" fontWeight="800" color={hasShortage ? '$terminalWarning' : '$terminalText'}>{material.owned}</TerminalText>
  ...
  return (
    <YStack gap={16} pb="$4">
      ...
      <TerminalPanel p="$3" flexDirection="row" items="center" justify="space-between" gap="$3">
        ...
        <YStack px="$3" py="$2" bg="$terminalCyanSoft"><MonoText size="$1" color="$terminalCyan">{t('inventory.autoPlan')}</MonoText></YStack>
      </TerminalPanel>
