  ...
  return (
    <Button unstyled p={0} borderWidth={0} items="stretch" pressStyle={{ opacity: 0.75 }} onPress={onToggle}>
      <TerminalPanel transition="300ms" tone={task.isCompleted ? 'success' : 'default'} p={12} flexDirection="row" items="center" gap={12}>
        <YStack width={28} height={28} items="center" justify="center" borderWidth={1} borderColor={task.isCompleted ? '$terminalSuccess' : '$terminalBorder'} bg={task.isCompleted ? '$terminalSuccessSoft' : 'transparent'}>
          {task.isCompleted ? <Check size={15} color={colors.terminalSuccess.val} /> : <Circle size={10} color={colors.terminalMuted.val} />}
        </YStack>
        <YStack grow={1} minW={0} gap="$1">
          <XStack items="center" justify="space-between" gap="$2">
            <TerminalText size="$3" lineHeight="$2.5" color={task.isCompleted ? '$terminalMuted' : '$terminalText'} textDecorationLine={task.isCompleted ? 'line-through' : 'none'} shrink={1} numberOfLines={1}>{task.label}</TerminalText>
            <MonoText size="$1">{task.completionProgress[0]}/{task.completionProgress[1]}</MonoText>
          </XStack>
          <MonoText size="$1" color="$terminalCyan">{task.reward}</MonoText>
    ...
    <YStack gap={16} pb="$4">
      ...
      <TerminalPanel cornerBrackets p={16}>
        <XStack items="flex-end" justify="space-between">
          <YStack><MonoText size="$1">{t('routine.progressToday')}</MonoText><TerminalText size="$8" fontWeight="900" color="$terminalCyan">{completedTaskCount}<TerminalText size="$4" color="$terminalMuted"> / {tasks.length}</TerminalText></TerminalText></YStack>
          <AnimatePresence>
