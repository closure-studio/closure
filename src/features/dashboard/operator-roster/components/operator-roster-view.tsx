      ...
      <XStack flexWrap="wrap" gap={8}>
        ...
        {visibleOperators.map((operator) => (
          <Button
            ...
            pressStyle={{ borderColor: '$appAccentBorder', opacity: 0.75 }}
            ...
            $xl={{ width: '18.5%' }}
          >
            <XStack justify="space-between"><OperatorRarity rarity={operator.rarity} /><MonoText size="$1" color="$appAccent">{t('operators.eliteLevel', { level: operator.elite })}</MonoText></XStack>
            <YStack mt={8}>
              <TerminalText size="$4" fontWeight="800">{operator.name}</TerminalText>
              <MonoText size="$1" color={operator.class === '医疗' ? '$appSuccess' : operator.class === '近卫' || operator.class === '先锋' ? '$appWarning' : '$appAccent'} numberOfLines={1}>{operator.class} · {operator.codename}</MonoText>
            </YStack>
            <XStack mt={8} justify="space-between"><MonoText size="$1">{t('operators.level', { level: operator.level, maxLevel: operator.maxLevel })}</MonoText><MonoText size="$1" color="$appAccent">{t('operators.skillLevel', { level: operator.skillLevel })}</MonoText></XStack>
          ...
          {selectedOperator ? (
            <TerminalPanel cornerBrackets px={20} pt={29} pb={20}>
              <XStack items="flex-start" justify="space-between">
                <YStack gap="$1"><OperatorRarity rarity={selectedOperator.rarity} /><TerminalText size="$6" fontWeight="900">{selectedOperator.name}</TerminalText><MonoText size="$2" color="$appAccent">{selectedOperator.codename} · {selectedOperator.class}</MonoText></YStack>
              ...
              <XStack mt={16} flexWrap="wrap" gap={8}>
                ...
                ].map(([label, value]) => <TerminalPanel key={label} width="31%" minW={0} grow={1} p={8} bg="$appSurfaceRaisedTranslucent"><MonoText size="$2">{label}</MonoText><TerminalText mt={2} size="$3">{value}</TerminalText></TerminalPanel>)}
              </XStack>
