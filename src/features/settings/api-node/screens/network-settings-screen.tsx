import { Activity, Circle, CircleDot, Clock3, RefreshCw, Route, Server } from 'lucide-react-native';
...
export function NetworkSettingsScreen({
  ...
  const selectedApiNode = displayNodes.find((apiNode) => apiNode.id === selectedApiNodeId)
    ?? displayNodes[0] ?? buildNode(API_NODE_HOSTS[0]);

  const handleNodeChange = (candidateNodeId: string) => {
    const matched = displayNodes.find((node) => node.id === candidateNodeId);
    if (matched && matched.id !== selectedApiNodeId) onSelectApiNode(matched.id);
  };
  ...
  const detectionStatus: DetectionStatus = isChecking ? 'checking' : selectedApiNode.outcome;
  const detectionTone = isChecking ? '$appWarning' : '$appAccent';
  const detectionSoft = isChecking ? '$appWarningSoft' : '$appAccentSoft';
  const detectionBorder = isChecking ? '$appWarningBorder' : '$appAccentBorder';
  const latencyRevealed = detectionStatus === 'reachable';
  const latencyValue = latencyRevealed ? selectedApiNode.latencyMs : '--';
  const latencyTone = latencyRevealed
    ? resolveLatencyTone(selectedApiNode.latencyMs)
    : isChecking ? '$appMuted' : '$appDanger';
    ...
    <SettingsPage header={(
      ...
      <YStack gap="$3" $md={{ gap: '$5' }}>
          ...
          <TerminalPanel cornerBrackets minH={320} overflow="hidden" p="$6" gap="$5">
            ...
            <XStack items="center" justify="space-between" gap="$3">
              <XStack items="center" gap="$2">
                ...
                <MonoText size="$1" color="$appAccent">{t('network.activeRoute')}</MonoText>
              </XStack>
            ...
            <XStack grow={1} flexDirection="column" justify="space-between" gap="$6" $md={{ flexDirection: 'row', items: 'flex-end', gap: '$8' }}>
              ...
              <YStack minW={180} items="flex-start" gap="$1" $md={{ items: 'flex-end' }}>
                ...
                <AnimatePresence mode="wait">
                  <XStack
                    key={isChecking ? 'checking' : `latency-${selectedApiNode.id}`}
                  ...
                  >
                    <TerminalText size="$10" lineHeight="$10" fontWeight="900" color={latencyTone} fontVariant={['tabular-nums']}>
                      {latencyValue}
                    </TerminalText>
                    <MonoText size="$3" color={latencyTone}>{t('network.latencyUnit')}</MonoText>
                  </XStack>
                </AnimatePresence>
              </YStack>
            </XStack>
        ...
        <YStack gap="$3">
          {layoutSize === 'large' ? (
            <XStack items="center" justify="space-between" gap="$3">
              <XStack items="baseline" gap="$2">
                ...
                <MonoText size="$1">{t('network.nodeCount')} / {t('network.nodesSubtitle')}</MonoText>
              </XStack>
              <Button
                unstyled
                minH="$4" px="$3"
                flexDirection="row" items="center" justify="center" gap="$2"
                borderWidth={1} borderColor={detectionBorder} bg={detectionSoft}
              ...
              >
                {isChecking ? <Spinner size="small" color="$appWarning" /> : <RefreshCw size={15} color={colors.appAccent.val} strokeWidth={1.8} />}
                <MonoText size="$2.5" color={detectionTone}>{t(isChecking ? 'network.checking' : 'network.retest')}</MonoText>
              </Button>
            </XStack>
          ) : null}

          <RadioGroup value={selectedApiNodeId} onValueChange={handleNodeChange} width="100%" aria-label={t('network.nodesTitle')}>
            <SlidingSelection
            ...
            >
              {displayNodes.map((apiNode, index) => {
                ...
                const latencyTone = isReachable ? resolveLatencyTone(apiNode.latencyMs) : '$appDanger';
                    ...
                    <RadioGroup.Item asChild unstyled id={`api-node-${apiNode.id}`} value={apiNode.id}>
                      ...
                      >
                        <TerminalText
                          ...
                          size="$9" lineHeight="$9" fontWeight="900"
                        ...
                        <XStack items="center" justify="space-between" gap="$3" z="$1">
                          <XStack items="center" gap="$2">
                            <SelectionIcon size={17} color={isSelected ? colors.appAccent.val : colors.appMuted.val} strokeWidth={1.8} />
                            <MonoText size="$1" color={isSelected ? '$appAccent' : '$appMuted'}>
                              {isSelected ? t('network.active') : t('network.switch')}
                            </MonoText>
                          </XStack>
                          <Server size={18} color={isSelected ? colors.appAccent.val : colors.appMuted.val} strokeWidth={1.6} />
                        </XStack>
                        ...
                        <XStack items="flex-end" justify="space-between" gap="$3" z="$1">
                          ...
                          <XStack shrink={0} items="baseline" gap="$1">
                            ...
                            <MonoText size="$1" color={!isChecking ? latencyTone : '$appMuted'}>{t('network.latencyUnit')}</MonoText>
                          </XStack>
                        </XStack>
          ...
          {queryError ? (
            <TerminalNotice tone="danger">{t('network.queryError')}</TerminalNotice>
          ) : null}
        </YStack>
      </YStack>
    </SettingsPage>
